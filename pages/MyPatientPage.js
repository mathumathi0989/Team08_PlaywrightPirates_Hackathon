export class MyPatientPage {
  constructor(page) {
    ...
    this.pageHeader = page
      .getByRole("heading", { name: /my patients/i })
      .first();
    ...
    this.viewTestReportsBtn = this.patientTable.getByRole("button", {
      name: /view previous test reports/i,
    });
    this.viewDietPlansBtn = this.patientTable.getByRole("button", {
      name: /view previous diet plans/i,
    });
    this.createNewReportPlanBtn = this.patientTable.getByRole("button", {
      name: /create new report\/plan/i,
    });
    this.editButton = this.patientTable.getByRole("button", { name: /Edit/i });
    this.deleteButton = this.patientTable.getByRole("button", {
      name: /Delete/i,
    });

    this.paginationText = page
      .locator(".pagination-info, [data-testid*='pagination' i], .pagination")
      .first();
    this.firstBtn = page
      .locator('button:has-text("<<"), button[aria-label*="first" i]')
      .first();
    this.prevBtn = page
      .locator('button:has-text("<"), button[aria-label*="previous" i]')
      .first();
    this.nextBtn = page
      .locator('button:has-text(">"), button[aria-label*="next" i]')
      .first();
    this.lastBtn = page
      .locator('button:has-text(">>"), button[aria-label*="last" i]')
      .first();
  }
